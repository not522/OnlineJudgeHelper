...
class SolutionC(Solution):
    ...
    def compile(self):
        return subprocess.call(['gcc', '-O2', '-o', self.get_a_out_name(),
                                '-Wno-deprecated', '-Wall',
...
class SolutionCxx(Solution):
    ...
    def compile(self):
        return subprocess.call(['g++', '-O2', '-o', self.get_a_out_name(),
                                '-Wno-deprecated', '-Wall', '-std=c++11',
                                '-I' + os.environ['LIB_PATH'],
                                self.source_file_name]) == 0
...
class SolutionHaskell(Solution):
    ...
    def compile(self):
        return subprocess.call(['ghc', '-o', self.get_a_out_name(),
                                self.source_file_name]) == 0
...
class SolutionScala(Solution):
    ...
    def compile(self):
        return subprocess.call(['scalac', self.source_file_name],
                               env=self.get_execute_env()) == 0
...
class SolutionCs(Solution):
    ...
    def compile(self):
        s = subprocess.check_output(['cygpath', '-w',
                                     self.source_file_name]).rstrip()
...
class SolutionGo(Solution):
    ...
    def compile(self):
        return subprocess.call(['go', 'build', '-o', 'a.out',
                                self.source_file_name]) == 0
...
class SolutionD(Solution):
    ...
    def compile(self):
        return subprocess.call(['dmd', '-m64', '-w', '-wi', '-O', '-release',
                                '-inline', '-ofa.out',
                                self.source_file_name]) == 0
...
class SolutionOCaml(Solution):
    ...
    def compile(self):
        return subprocess.call(['ocamlc', '-o', self.get_a_out_name(),
                                self.source_file_name]) == 0
